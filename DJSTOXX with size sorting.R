...
# Files with data
file_names = c("DJSTOXX_filter.csv",
               "DJSTOXX_stock_returns.csv",
               "DJSTOXX_benchmark_returns.csv",
               "DJSTOXX_market_cap.csv",
               "DJSTOXX_price_to_book_ratio.csv"
)
...
esg_scores_df = read.csv(file = paste0(wd,"DJSTOXX_scores_monthly.csv"),header = F, sep =";", dec = ",")
header_df = read.csv(file = paste0(wd,"DJSTOXX_header.csv"),header = F, sep =";", dec = ",")
...
# Make values numeric
esg_scores_dec_df = as.data.frame(apply(esg_scores_dec, 2, as.numeric))
...
for(i in 1:length(years)){
  ...
  for (j in 1:nrow(stock_returns_curr_year)){
    ...
    # Include size
    stocks_num_in_size_group_esgcs = length(stock_codes_with_esgcs) %/% 2
    stocks_num_in_esgcs_portfolio_in_size_group = stocks_num_in_size_group_esgcs %/% 5
    stocks_num_in_esgcs_portfolio = 2 * stocks_num_in_esgcs_portfolio_in_size_group
    
    size_score_curr_week_with_esgcs = size_score_curr_week[stock_codes_with_esgcs]
    
    small_size_score_for_esgcs_indices =  which(size_score_curr_week_with_esgcs <= sort(size_score_curr_week_with_esgcs, decreasing=F)[stocks_num_in_size_group_esgcs], arr.ind=TRUE)[1:stocks_num_in_size_group_esgcs]                                     
    large_size_score_for_esgcs_indices =  which(size_score_curr_week_with_esgcs > sort(size_score_curr_week_with_esgcs, decreasing=F)[stocks_num_in_size_group_esgcs], arr.ind=TRUE)[1:stocks_num_in_size_group_esgcs]                                     
    
    esgcs_small_size_score = esgcs_curr_week[names(small_size_score_for_esgcs_indices)]
    esgcs_large_size_score = esgcs_curr_week[names(large_size_score_for_esgcs_indices)]
    

    for(l in 1:portfolios_num){
      
      # Find constituents of portfolio 
      portfolio_esgcs_indices_small_size_score = which(esgcs_small_size_score <= sort(esgcs_small_size_score, decreasing=F)[stocks_num_in_esgcs_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_esgcs_portfolio_in_size_group]
      portfolio_esgcs_indices_large_size_score = which(esgcs_large_size_score <= sort(esgcs_large_size_score, decreasing=F)[stocks_num_in_esgcs_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_esgcs_portfolio_in_size_group]
      
      esgcs_portfolio_l_stock_codes  = c(names(portfolio_esgcs_indices_small_size_score), names(portfolio_esgcs_indices_large_size_score))
      esgcs_portfolio_l = esgcs_curr_week[esgcs_portfolio_l_stock_codes]
      ...
      esgcs_small_size_score[esgcs_portfolio_l_stock_codes] = 1000000
      esgcs_large_size_score[esgcs_portfolio_l_stock_codes] = 1000000
    }  
    ...
    # Include size
    stocks_num_in_size_group_esg = length(stock_codes_with_esg) %/% 2
    stocks_num_in_esg_portfolio_in_size_group = stocks_num_in_size_group_esg %/% 5
    stocks_num_in_esg_portfolio = 2 * stocks_num_in_esg_portfolio_in_size_group
    
    size_score_curr_week_with_esg = size_score_curr_week[stock_codes_with_esg]
    
    small_size_score_for_esg_indices =  which(size_score_curr_week_with_esg <= sort(size_score_curr_week_with_esg, decreasing=F)[stocks_num_in_size_group_esg], arr.ind=TRUE)[1:stocks_num_in_size_group_esg]                                     
    large_size_score_for_esg_indices =  which(size_score_curr_week_with_esg > sort(size_score_curr_week_with_esg, decreasing=F)[stocks_num_in_size_group_esg], arr.ind=TRUE)[1:stocks_num_in_size_group_esg]                                     
    
    esg_small_size_score = esg_curr_week[names(small_size_score_for_esg_indices)]
    esg_large_size_score = esg_curr_week[names(large_size_score_for_esg_indices)]
    
    for(l in 1:portfolios_num){
      
      # Find constituents of portfolio 
      portfolio_esg_indices_small_size_score = which(esg_small_size_score <= sort(esg_small_size_score, decreasing=F)[stocks_num_in_esg_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_esg_portfolio_in_size_group]
      portfolio_esg_indices_large_size_score = which(esg_large_size_score <= sort(esg_large_size_score, decreasing=F)[stocks_num_in_esg_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_esg_portfolio_in_size_group]
      
      esg_portfolio_l_stock_codes  = c(names(portfolio_esg_indices_small_size_score), names(portfolio_esg_indices_large_size_score))
      esg_portfolio_l = esg_curr_week[esg_portfolio_l_stock_codes]
      ...
      # Replace esg scores in portfolio with very big number
      esg_small_size_score[esg_portfolio_l_stock_codes] = 1000000
      esg_large_size_score[esg_portfolio_l_stock_codes] = 1000000
    }
    ...
    # Include size
    stocks_num_in_size_group_e = length(stock_codes_with_e) %/% 2
    stocks_num_in_e_portfolio_in_size_group = stocks_num_in_size_group_e %/% 5
    stocks_num_in_e_portfolio = 2 * stocks_num_in_e_portfolio_in_size_group
    
    size_score_curr_week_with_e = size_score_curr_week[stock_codes_with_e]
    
    small_size_score_for_e_indices =  which(size_score_curr_week_with_e <= sort(size_score_curr_week_with_e, decreasing=F)[stocks_num_in_size_group_e], arr.ind=TRUE)[1:stocks_num_in_size_group_e]                                     
    large_size_score_for_e_indices =  which(size_score_curr_week_with_e > sort(size_score_curr_week_with_e, decreasing=F)[stocks_num_in_size_group_e], arr.ind=TRUE)[1:stocks_num_in_size_group_e]                                     
    
    e_small_size_score = e_curr_week[names(small_size_score_for_e_indices)]
    e_large_size_score = e_curr_week[names(large_size_score_for_e_indices)]
    
    for(l in 1:portfolios_num){
      
      # Find constituents of portfolio 
      portfolio_e_indices_small_size_score = which(e_small_size_score <= sort(e_small_size_score, decreasing=F)[stocks_num_in_e_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_e_portfolio_in_size_group]
      portfolio_e_indices_large_size_score = which(e_large_size_score <= sort(e_large_size_score, decreasing=F)[stocks_num_in_e_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_e_portfolio_in_size_group]
      
      e_portfolio_l_stock_codes  = c(names(portfolio_e_indices_small_size_score), names(portfolio_e_indices_large_size_score))
      e_portfolio_l = e_curr_week[e_portfolio_l_stock_codes]
      ...
      # Replace e scores in portfolio with very big number
      e_small_size_score[e_portfolio_l_stock_codes] = 1000000
      e_large_size_score[e_portfolio_l_stock_codes] = 1000000
    ...
    # Include size
    stocks_num_in_size_group_s = length(stock_codes_with_s) %/% 2
    stocks_num_in_s_portfolio_in_size_group = stocks_num_in_size_group_s %/% 5
    stocks_num_in_s_portfolio = 2 * stocks_num_in_s_portfolio_in_size_group
    
    size_score_curr_week_with_s = size_score_curr_week[stock_codes_with_s]
    
    small_size_score_for_s_indices =  which(size_score_curr_week_with_s <= sort(size_score_curr_week_with_s, decreasing=F)[stocks_num_in_size_group_s], arr.ind=TRUE)[1:stocks_num_in_size_group_s]                                     
    large_size_score_for_s_indices =  which(size_score_curr_week_with_s> sort(size_score_curr_week_with_s, decreasing=F)[stocks_num_in_size_group_s], arr.ind=TRUE)[1:stocks_num_in_size_group_s]                                     
    
    s_small_size_score = s_curr_week[names(small_size_score_for_s_indices)]
    s_large_size_score = s_curr_week[names(large_size_score_for_s_indices)]
    
    for(l in 1:portfolios_num){
      
      # Find constituents of portfolio 
      portfolio_s_indices_small_size_score = which(s_small_size_score <= sort(s_small_size_score, decreasing=F)[stocks_num_in_s_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_s_portfolio_in_size_group]
      portfolio_s_indices_large_size_score = which(s_large_size_score <= sort(s_large_size_score, decreasing=F)[stocks_num_in_s_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_s_portfolio_in_size_group]
      
      s_portfolio_l_stock_codes  = c(names(portfolio_s_indices_small_size_score), names(portfolio_s_indices_large_size_score))
      s_portfolio_l = s_curr_week[s_portfolio_l_stock_codes]
      ...
      # Replace esg scores in portfolio with very big number
      s_small_size_score[s_portfolio_l_stock_codes] = 1000000
      s_large_size_score[s_portfolio_l_stock_codes] = 1000000
    ...
    # Include size
    stocks_num_in_size_group_g = length(stock_codes_with_g) %/% 2
    stocks_num_in_g_portfolio_in_size_group = stocks_num_in_size_group_g %/% 5
    stocks_num_in_g_portfolio = 2 * stocks_num_in_g_portfolio_in_size_group
    
    size_score_curr_week_with_g = size_score_curr_week[stock_codes_with_g]
    
    small_size_score_for_g_indices =  which(size_score_curr_week_with_g <= sort(size_score_curr_week_with_g, decreasing=F)[stocks_num_in_size_group_g], arr.ind=TRUE)[1:stocks_num_in_size_group_g]                                     
    large_size_score_for_g_indices =  which(size_score_curr_week_with_g > sort(size_score_curr_week_with_g, decreasing=F)[stocks_num_in_size_group_g], arr.ind=TRUE)[1:stocks_num_in_size_group_g]                                     
    
    g_small_size_score = g_curr_week[names(small_size_score_for_g_indices)]
    g_large_size_score = g_curr_week[names(large_size_score_for_g_indices)]
    
    for(l in 1:portfolios_num){
      
      # Find constituents of portfolio 
      portfolio_g_indices_small_size_score = which(g_small_size_score <= sort(g_small_size_score, decreasing=F)[stocks_num_in_g_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_g_portfolio_in_size_group]
      portfolio_g_indices_large_size_score = which(g_large_size_score <= sort(g_large_size_score, decreasing=F)[stocks_num_in_g_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_g_portfolio_in_size_group]
      
      g_portfolio_l_stock_codes  = c(names(portfolio_g_indices_small_size_score), names(portfolio_g_indices_large_size_score))
      g_portfolio_l = g_curr_week[g_portfolio_l_stock_codes]
      ...
      # Replace esg scores in portfolio with very big number
      g_small_size_score[g_portfolio_l_stock_codes] = 1000000
      g_large_size_score[g_portfolio_l_stock_codes] = 1000000
    ...
    # Include size
    stocks_num_in_size_group_equal_esg = length(stock_codes_with_equal_esg) %/% 2
    stocks_num_in_equal_esg_portfolio_in_size_group = stocks_num_in_size_group_equal_esg %/% 5
    stocks_num_in_equal_esg_portfolio = 2 * stocks_num_in_equal_esg_portfolio_in_size_group
    
    size_score_curr_week_with_equal_esg = size_score_curr_week[stock_codes_with_equal_esg]
    
    small_size_score_for_equal_esg_indices =  which(size_score_curr_week_with_equal_esg <= sort(size_score_curr_week_with_equal_esg, decreasing=F)[stocks_num_in_size_group_equal_esg], arr.ind=TRUE)[1:stocks_num_in_size_group_equal_esg]                                     
    large_size_score_for_equal_esg_indices =  which(size_score_curr_week_with_equal_esg > sort(size_score_curr_week_with_equal_esg, decreasing=F)[stocks_num_in_size_group_equal_esg], arr.ind=TRUE)[1:stocks_num_in_size_group_equal_esg]                                     
    
    equal_esg_small_size_score = equal_esg_curr_week[names(small_size_score_for_equal_esg_indices)]
    equal_esg_large_size_score = equal_esg_curr_week[names(large_size_score_for_equal_esg_indices)]
    
    for(l in 1:portfolios_num){
      
      # Find constituents of portfolio 
      portfolio_equal_esg_indices_small_size_score = which(equal_esg_small_size_score <= sort(equal_esg_small_size_score, decreasing=F)[stocks_num_in_equal_esg_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_equal_esg_portfolio_in_size_group]
      portfolio_equal_esg_indices_large_size_score = which(equal_esg_large_size_score <= sort(equal_esg_large_size_score, decreasing=F)[stocks_num_in_equal_esg_portfolio_in_size_group], arr.ind=TRUE)[1:stocks_num_in_equal_esg_portfolio_in_size_group]
      
      equal_esg_portfolio_l_stock_codes  = c(names(portfolio_equal_esg_indices_small_size_score), names(portfolio_equal_esg_indices_large_size_score))
      equal_esg_portfolio_l = equal_esg_curr_week[equal_esg_portfolio_l_stock_codes]
     ...
     # Calculate portfolio return current week
      ...
      # Replace esg scores in portfolio with very big number
      equal_esg_small_size_score[equal_esg_portfolio_l_stock_codes] = 1000000
      equal_esg_large_size_score[equal_esg_portfolio_l_stock_codes] = 1000000
    }
...
plot(cum_esgcs_portfolio_returns, legend.loc = "left", main = "DJSTOXX ESGCS Portfolios")
plot(cum_e_portfolio_returns, legend.loc = "left", main = "DJSTOXX Environmental Portfolios")
plot(cum_s_portfolio_returns, legend.loc = "left", main = "DJSTOXX Social Portfolios")
plot(cum_g_portfolio_returns, legend.loc = "left", main = "DJSTOXX Corporate Governance Portfolios")
plot(cum_esg_portfolio_returns, legend.loc = "left", main = "DJSTOXX ESG Portfolios")
plot(cum_equal_esg_portfolio_returns, legend.loc = "left", main = "DJSTOXX Equal ESG Portfolios")

plot(esgcs_avg_esg_score, legend.loc = "left", main = "DJSTOXX ESGCS Average Score")
plot(e_avg_esg_score, legend.loc = "left", main = "DJSTOXX Environmental Average Score")
plot(s_avg_esg_score, legend.loc = "left", main = "DJSTOXX Social Average Score")
plot(g_avg_esg_score, legend.loc = "left", main = "DJSTOXX Corporate Governance Average Score")
plot(esg_avg_esg_score, legend.loc = "left", main = "DJSTOXX ESG Average Score")
plot(equal_esg_avg_esg_score, legend.loc = "left", main = "DJSTOXX Equal ESG Average Score")

plot(esgcs_avg_size_score, legend.loc = "left", main = "DJSTOXX ESGCS Average Size Score")
plot(e_avg_size_score, legend.loc = "left", main = "DJSTOXX Environmental Average Size Score")
plot(s_avg_size_score, legend.loc = "left", main = "DJSTOXX Social Average Size Score")
plot(g_avg_size_score, legend.loc = "left", main = "DJSTOXX Corporate Governance Average Size Score")
plot(esg_avg_size_score, legend.loc = "left", main = "DJSTOXX ESG Average Size Score")
plot(equal_esg_avg_size_score, legend.loc = "left", main = "DJSTOXX Equal ESG Average Size Score")

plot(esgcs_avg_ptb_score, legend.loc = "left", main = "DJSTOXX ESGCS Average PTB Score")
plot(e_avg_ptb_score, legend.loc = "left", main = "DJSTOXX Environmental Average PTB Score")
plot(s_avg_ptb_score, legend.loc = "left", main = "DJSTOXX Social Average PTB Score")
plot(g_avg_ptb_score, legend.loc = "left", main = "DJSTOXX Corporate Governance Average PTB Score")
plot(esg_avg_ptb_score, legend.loc = "left", main = "DJSTOXX ESG Average PTB Score")
plot(equal_esg_avg_ptb_score, legend.loc = "left", main = "DJSTOXX Equal ESG Average PTB Score")
...
plot(all_best_stocks_num, main = "DJSTOXX Number of all best companies")

plot(stocks_num_with_esg_score, legend.loc = "left", main = "DJSTOXX Number of companies with esg score")
...
save.image(file=paste0(wd, "DJSTOXX with size sorting.RData"))
