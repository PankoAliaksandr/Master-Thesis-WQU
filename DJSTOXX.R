...
esg_mscieurope_sa = read.csv(paste0(wd, "MSCIEUROPESTD_esg.csv"))
rownames(esg_mscieurope_sa) = esg_mscieurope_sa$Date
esg_mscieurope_sa$Date = NULL
colnames(esg_mscieurope_sa) = substring(colnames(esg_mscieurope_sa), 2)

# Replace zeros with NA and delete rows with all NA
esg_mscieurope_sa[esg_mscieurope_sa == 0] = NA
esg_mscieurope_sa = esg_mscieurope_sa[apply(esg_mscieurope_sa,1,function(x)any(!is.na(x))),]

# Convert weekly to monthly data keeping only the last observation for every month
esg_mscieurope_sa$year_month = paste0(year(rownames(esg_mscieurope_sa)), month(rownames(esg_mscieurope_sa)))
esg_mscieurope_sa = esg_mscieurope_sa[!duplicated(esg_mscieurope_sa[, "year_month"], fromLast=T),]

# Create xts object
esg_mscieurope_sa$year_month = NULL
esg_mscieurope_sa = xts(x = esg_mscieurope_sa, order.by = as.Date(rownames(esg_mscieurope_sa)))

# Analyzed period for correlation
start_date_esg_mscieurope_sa = "2009-12-01"
esg_mscieurope_sa = window(x = esg_mscieurope_sa, start = start_date_esg_mscieurope_sa, end = end_date_esg)

#  Create 6 xts objects for 6 diff  monthly scores
esg_scores_monthly = paste0(esg_scores, "_monthly")

for(i in 1:esg_scores_num){
  # Monthly esg scores
  xts_object = get(esg_scores[i])
  xts_object_monthly = window(x = xts_object, start = start_date_esg_mscieurope_sa, end = end_date_esg)
...
# Read Firm Info for MSCI EUROPE
mscieurope_firm_info = readRDS(paste0(wd, "MSCIEUROPESTD_firm_info.rds"))

# Read Firm Info for DJSTOXX
djstoxx_firm_info = readRDS(paste0(wd, "DJSTOXX_firm_info.rds"))

# Merge Firm Info dataframes for djstoxx and MSCI EUROPE
all_firm_info = merge(x = djstoxx_firm_info, y = mscieurope_firm_info, by.x = "ISIN", by.y = "MSISIN", suffixes = c(".DJ", ".MSCI"), all.x = TRUE)
all_firm_info$matched = !is.na(all_firm_info$NAME.MSCI)

# Keep only rows that matched by ISIN in both djstoxx and MSCI EUROPE
all_firm_info_reduced = all_firm_info[all_firm_info$matched == TRUE,]

# Replace Sustainanalytics code with ISIN in colnames
colnames(esg_mscieurope_sa) = unlist(mscieurope_firm_info[, "MSISIN"])

# Create data frame with correlations
correlation = data.frame(matrix(NA, nrow = nrow(esg_mscieurope_sa), ncol = esg_scores_num))
correlation = xts(correlation, order.by = index(esg_mscieurope_sa))
...
for(i in 1:esg_scores_num){
  ...
  ind_in_all_firm_info_reduced = which(colnames(xts_object) %in% all_firm_info_reduced$DSCD.DJ)
  xts_object = xts_object[,ind_in_all_firm_info_reduced]
  
  # Replace Datastream code with MSISIN in colnames
  ind_in_djstoxx_firm_info = match(colnames(xts_object), rownames(djstoxx_firm_info))
  colnames(xts_object) = unlist(djstoxx_firm_info[ind_in_djstoxx_firm_info, "ISIN"])
  
  # Keep corresponding columns only
  esg_mscieurope_sa_current = esg_mscieurope_sa[,colnames(xts_object)]
  
  # Compute correlation
  correlation[, i] = diag(cor(t(xts_object), t(esg_mscieurope_sa_current), use = "pairwise.complete.obs", method = "spearman"))
...
plot(correlation, legend.loc = "left", main = "Correlation between Datastream and Sustainanalytics ESG scores (djstoxx)")
...
save.image(file=paste0(wd, "DJSTOXX.RData"))
